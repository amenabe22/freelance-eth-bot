...
export const postAJobWorkingLocationHandler = Telegraf.on(["text", "contact", "document", "photo"], async (ctx: any) => {
    if (ctx.message.text) {
        if (ctx.message.text == "Skip") {
            ...
            await ctx.replyWithHTML(`Almost done ${ctx.from.first_name}, please enter vacancy number.`, kb.postAJobOptionalKeyboard);
            return ctx.wizard.next();
        }
    } else {
...
export const postAJobVancancyNumberHandler = Telegraf.on(["text", "contact", "document", "photo"], async (ctx: any) => {
    if (ctx.message.text){
        if(ctx.message.text == "Skip"){
            ctx.scene.state.postAJobVancancyNumber = " ";
            ctx.replyWithHTML(`please enter Gender for applicants`, {
                reply_markup: {
                    keyboard: [
  ...
  return ctx.wizard.next();
        ...
        ctx.replyWithHTML("please enter a valid vacancy number.", kb.postAJobOptionalKeyboard);
        return
    }
})
export const postAJobApplicantGenderHandler = Telegraf.on(["text", "contact", "document", "photo"], async (ctx: any)=>{
    if(ctx.message.text){
         if(ctx.message.text == "male" || ctx.message.text == "female" || ctx.message.text == "both"){
            ctx.scene.state.postAJobApplicantGender = ctx.message.text;
            ctx.replyWithHTML(`please enter job close date in the format of dd/mm/yyyy`, kb.postAJobOptionalKeyboard);
            return ctx.wizard.next();
        }else{
            ctx.replyWithHTML("please enter a valid gender!",{
                reply_markup: {
                    keyboard: [
                        [{text: "male"}, {text: "female"}],
                        [{text: "both"}],
...
export const postAJobCloseDateHandler = Telegraf.on(["text", "contact", "document", "photo"], async (ctx: any)=>{
    if(ctx.message.text){
        if(ctx.message.text == "Skip"){
            ctx.scene.state.postAJobCloseData = " ";
            ctx.replyWithHTML(`please enter applicant needed for your job`, kb.postAJobOptionalKeyboard);
        }else{
          ctx.scene.state.postAJobCloseData = ctx.message.text;
          ctx.replyWithHTML(`please enter applicant needed for your job`, kb.postAJobOptionalKeyboard);
        }
        return ctx.wizard.next();
    }else{
        ctx.replyWithHTML("please enter valid close date!", kb.postAJobOptionalKeyboard);
        return
    }
})
...
export const jobPostConfirmHandler = async (ctx: any) => {
    ...
    const { data } = await inserJobPost({
        object: {
            ...
            gender: postAJobApplicantGender,
            vacancy_number: postAJobVancancyNumber,
            applicant_number: postAJobApplicantNeeded,
            close_data: postAJobCloseData,
            sectors: {
                data: {
...
export const reviewEmployeeInitHandler = async (ctx: any) => {
    ...
    globalState.reviewAnswers = []
    globalState.reviewQuestions = []
    if (data) {
        const questions = data.job_seeker_review_questions;
        console.log("QUESTIONS: ", questions)
        // ctx.reply("")
        // set questions if they're not in the state
        globalState.reviewQuestions = questions
        // if (!globalState.reviewQuestions) {
        //     globalState.reviewQuestions = questions
        // }
        // ctx.reply(globalState.reviewQuestions[0].question)
        ...
        // // slice of first item from questions array
        // globalState.reviewQuestions = globalState.reviewQuestions.slice(1)
    }

    ctx.replyWithHTML(`How you rate the skill of your employee [${ctx.session.applicantFirstName} ${ctx.session.applicantLastName}]`, kb.ratingKeyboard);
}
export const reviewEmployeeQAnswerHandler = Telegraf.on(["photo", "text", "contact", "document"], async (ctx: any) => {
    if (ctx.message.text) {
        console.log("Ans: ", ctx.message.text)
        // use a stack approach to remove first item on each iteration
        if (globalState.reviewQuestions.length) {
            console.log("Ansx: ", ctx.message.text)
            ctx.reply(globalState.reviewQuestions[0].question)
            globalState.reviewAnswers.push({
                "question": globalState.reviewQuestions[0],
                "answer": ctx.message.text
            })
            globalState.reviewQuestions = globalState.reviewQuestions.slice(1)
        } else {
            // means all questions have been answered so send success response
            ctx.reply("Done")
            console.log("Answers: ", globalState.reviewAnswers)
            ctx.scene.leave()
        }
    }
})
